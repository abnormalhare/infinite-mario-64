...
const current_version : String = "v0.4"
...
func try_submit_save_block(in_seed : String, in_star_id : String = "", in_time : float = 0, in_coins : int = 0, in_red_coins : int = 0, in_checkpoints_used : int = 0, in_wants_save : bool = false) -> void:
	...
	if save_blocks_lookup.has(in_seed):
		...
		save_blocks[save_blocks_lookup[in_seed]].try_add_star_data_entry(in_star_id, in_time, in_coins, in_red_coins, in_checkpoints_used)
	else:
		#print("we don't have this seed, so let's make a new block and submit the star data entry to it")
		var new_block := SaveBlock.new(in_seed)
		new_block.try_add_star_data_entry(in_star_id, in_time, in_coins, in_red_coins, in_checkpoints_used)
		save_blocks_lookup[in_seed] = save_blocks.size()
		save_blocks.append(new_block)
	if in_wants_save:
		save_game()
...
func save_game():
	...
	for entry in save_blocks:
		...
		save_bytes.put_u32(entry.red_coins)
	...
	match in_version:
		"v0.1":
			...
			for block_num in block_count:
				...
				for star_data in star_data_entries:
					...
					try_submit_save_block(seed_string, star_id_string, star_time, coin_count, 0, 0)
					#print("ack")
		...
		"v0.2":
			...
			for block_num in block_count:
				...
				for star_data in star_data_entries:
					...
					#new_star_data.time = star_time
					#new_block.star_data[star_id_string] = new_star_data
					try_submit_save_block(seed_string, star_id_string, star_time, coin_count, 0, star_checkpoints)
				#save_blocks[new_block.seed] = new_block
		"v0.3":
		...
		"v0.4":
			...
			for block_num in block_count:
				...
				var seed_string : String = save.get_buffer(seed_length).get_string_from_utf8()
				var coin_count : int = save.get_32()
				var red_coin_count : int = save.get_32()
				var star_data_entries : int = save.get_32()
				for star_data in star_data_entries:
					var star_id_string_length : int = save.get_32()
					var star_id_string : String = save.get_buffer(star_id_string_length).get_string_from_utf8()
					...
					#new_star_data.time = star_time
					#new_block.star_data[star_id_string] = new_star_data
					try_submit_save_block(seed_string, star_id_string, star_time, coin_count, red_coin_count, star_checkpoints)
				#save_blocks[new_block.seed] = new_block
...
func get_total_coin_count():
	var coin_count : int = 0
	for entry in save_blocks:
		coin_count += entry.coins
	return coin_count
	
func get_total_red_coin_count():
	var red_coin_count : int = 0
	for entry in save_blocks:
		red_coin_count += entry.red_coins
	return red_coin_count
